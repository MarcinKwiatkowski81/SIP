...
"""Write Transport.cpp to the SIP project."""
import os

base = "/home/mkwiatkowski/Projects/SIP"
os.makedirs(f"{base}/src", exist_ok=True)

transport_h = r"""// Transport.h – SIP dual-transport layer (UDP + TCP, RFC 3261 §18)
//
// One UDP socket + one TCP listen socket, both bound to the same port.
// TCP connections are pooled (MAX_TCP slots).  Outbound connections are
// established lazily on first sendTcp(); inbound connections are accepted
// by the TCP accept thread.  Both flavours deliver fully-assembled SIP
// messages via a single RecvFn callback.
//
// TCP framing: Content-Length header determines body size (RFC 3261 §18.3).
// Retransmission timers in Transaction are disabled for TCP (isUdp = false).
#pragma once
#include "common.h"
#include <functional>
#include <pthread.h>
...
// Receive callback – called from UDP or TCP reader thread.
// data/len are valid only for the duration of the call.
...
transport_cpp = r"""// Transport.cpp – UDP + TCP dual-transport for SIP (RFC 3261 §18)
#include "Transport.h"
#include <cstdio>
#include <cstring>
#include <cstdlib>
...
with open(f"{base}/include/Transport.h", "w") as f:
    f.write(transport_h)
print(f"Transport.h: {len(transport_h)} bytes")

with open(f"{base}/src/Transport.cpp", "w") as f:
    f.write(transport_cpp)
print(f"Transport.cpp: {len(transport_cpp)} bytes")
